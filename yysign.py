...
# -------------------------------
# cron "0 0 8,20 * * *" script-path=xxx.py,tag=匹配cron用
# const $ = new Env('雨云签到续期');


##变量雨云账号密码 注册地址https://www.rainyun.com/MTU4MTk4_  登录后积分中心里面 赚钱积分 (如绑定微信 直接就有2000分）就可以用积分兑换主机 需要每天晚上八点蹲点

import json ,requests ,os ,time #line:8
yyusername =os .getenv ("yyusername")#line:12
yypassword =os .getenv ("yypassword")#line:13
yyproduct_id =os .getenv ("yyproduct_id")
